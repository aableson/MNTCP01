...
This is actually a fair approximation to the solution, since $e^{0.5}
+ 0.5 2.149$ which is close to what the equation RHS is, 2.
...
\item
  \begin{Question}
    ...
    solution estimates generated by Newton's method). \par
    \par \end{Question}
  \begin{Solution}
...
\item
  \begin{Question}
    Use Newton's Method with the function \(x^3=5\) and initial
    value \(x_0=1.5\) to calculate \(x_1 , x_2 , x_3\) (the next three
    solution estimates generated by Newton's method). \par
...
\par 
Moving everything to the left side, we get $$ \underbrace{x^3 - 5}_{f(x)}  = 0$$
Differentiating, we have \(f'(x)= 3x^2\).  Therefore: \par
\(\ds x_1 = x_0 - \frac{f(x_0)}{f'(x_0)}= x_0 - \frac{x_0^3-5}{3
  x_0^2} = 1.5 - \frac{1.5^3 - 5}{3 * 1.5^2} = 1.74074\) \par
\(\ds x_2 = x_1 - \frac{f(x_1)}{f'(x_1)}= 1.74074 - \frac{1.74074^3 -
  5}{3 \times 1.74074^2} \approx 1.71052\) \par
\(\ds x_3 = x_2 - \frac{f(x_2)}{f'(x_2)}= 1.71052 - \frac{1.71052^3 -
  5}{3 \times 1.71052^2} \approx 1.70998\) \par
\par
This sequence provides successive approximations to the third root of
5, \(\sqrt[3]{5} \approx 1.70997\)
\par\end{Solution}
% ***************************************************************
\item
  \begin{Question}
    ...
    satisfy, and use Newton's method to solve that.)
    \par
    \par \end{Question}
  \begin{Solution}
...
Method. \leavevmode\\\relax We let \(f(x) = x^3-4\), thus \(f'(x) =
3x^2\). \leavevmode\\\relax A good initial guess could be \(x_0 =
8^{1/3}=2\).  Thus: \par
\(x_1 = x_0 - \frac{f(x_0)}{f'(x_0)}= 2 - \frac{2^3 - 4}{3 \times 2^2}
= 1.66667\) \par
\(x_2 = x_1 - \frac{f(x_1)}{f'(x_1)}= 1.66667 - \frac{1.66667 ^3 -
  4}{3 \times 1.66667^2} \approx 1.59111\) \par
\(x_3 = x_2 - \frac{f(x_2)}{f'(x_2)}= 1.59111 - \frac{1.59111 ^3 -
  4}{3 \times 1.59111^2} \approx 1.58741\) \par
\(x_4 = x_3 - \frac{f(x_3)}{f'(x_3)}= 1.58741 - \frac{1.58741 ^3 -
  4}{3 \times 1.58741^2} \approx 1.5874\) \par
Using a calculator to find \(4^{\frac{1}{3}}\), we get:
\(4^{\frac{1}{3}}\approx 1.58740\).
\par\end{Solution}
\end{multicols}
